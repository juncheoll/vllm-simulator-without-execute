...
class Request():
    def __init__(self, request_id:str, input_text:str, sampling_params:SamplingParams):
...
def create_test_prompts() -> List[Request]:
    ...
    input_text_list = csv.get_rows_input_text()
    sampling_params = SamplingParams(temperature=0, min_tokens=min_tokens, max_tokens=max_tokens)
    
    requests:List[Request] = []
    
    for request_id, input_text in zip(request_id_list, input_text_list):
        requests.append(Request(request_id=request_id, input_text=input_text, sampling_params=sampling_params))
...
def process_requests(engine: LLMEngine,
    ...
    for request in requests:
        request_id, text, sampling_params = request.request_id, request.input_text, request.sampling_params
        engine.add_request(request_id, text, sampling_params)
