...
class LLMEngine:
    ...
    def step(self) : # -> List[Union[RequestOutput, EmbeddingRequestOutput]]:
        ...
        if not self.has_unfinished_requests():
            ...
            print(f'total_step_time : {self.total_step_time:.10f}')
        ...
        if start_step_time != 0 and end_step_time != 0:
            print(f'step_time : {step_time:.10f}')
        else:
            print('step_time error!!!!!!')
        if start_schedule_time != 0 and end_schedule_time != 0:
            print(f'schedule_time : {schedule_time:.10f}')
        else:
            print('schedule_time error!!!!!!')
        if start_execute_time != 0 and end_execute_time != 0:
            print(f'execute_time : {execute_time:.10f}')  
        else:
            print('execute_time error!!!!!!')
    ...
    def simulate_execute_model(self, csv, execute_model_req: ExecuteModelRequest) -> List[SamplerOutput] | None:
        samplerOutput = SamplerOutput(outputs=[])
        for seq_group_metadata in execute_model_req.seq_group_metadata_list:
            output = CompletionSequenceGroupOutput(samples=[], prompt_logprobs=None)
            parent_seq_id = int(seq_group_metadata.request_id)
            
            for i, seq_data in seq_group_metadata.seq_data.items():
            ...
            samplerOutput.outputs.append(output)
...
class CSV_Assist:
    ...
    def read_csv(self, file_path):
        ...
        self.df['Output_Tokens'] = self.df['Output_Tokens'].apply(json.loads)
        print(f"'{file_path}' 파일을 성공적으로 읽어들임.")
        
        
    def get_next_token(self, request_id: int, len_prompt_tokens: int, len_generated_tokens: int):
        output_Tokens = self.df.loc[request_id, 'Output_Tokens']
        output_token_id = output_Tokens[len_generated_tokens]
        decoded_token = self.tokenizer.decode(output_token_id)
        
        return output_token_id, decoded_token
    
    def get_input_text(self, request_id: int):
        return self.df.loc[request_id, 'Input_Content']
    
    def get_output_text(self, request_id: int):
        return  self.df.loc[request_id, 'Output_Content']
    
    def get_input_tokens(self, request_id: int) -> List[int]:
        return self.df.loc[request_id, 'Input_Tokens']
    
    def get_output_tokens(self, request_id: int) -> List[int]:
        return self.df.loc[request_id, 'Output_Tokens']
    ...
    def get_rows_input_tokens(self):
        return self.df['Input_Tokens'].to_list()
    
    def get_rows_output_tokens(self):
        return self.df['Output_Tokens'].to_list()
